...
class FastSpeech(nn.Module):
    def __init__(self, n_mel_channels,
        ...
        self.future_predicter = nn.Sequential(nn.Linear(symbols_embedding_dim, symbols_embedding_dim),
                                                nn.ReLU(),
                                                nn.Linear(symbols_embedding_dim, symbols_embedding_dim))
        ...
        self.proj_future = nn.Linear(out_fft_output_size, n_mel_channels, bias=True)
        self.relu = nn.ReLU()
    ...
    def __init__(self, n_head, d_model, d_head, dropout, mask_type, separate_mem_attn, local_window_size, dropatt=0.1,
        ...
        if self.mask_type == 'tri':
            ...
            for shape in [400]:
                    ...
                    # import matplotlib.pyplot as plt
                    # plt.imshow(self.attn_mask[400][10])
                    # plt.savefig('bad')
                    # exit()
            ...
            if self.mask_type == 'default':
                ...
                if use_memory_mask:
                    ...
                    for i in range(len(attn_mask)):
                        attn_mask[i, max(0, i-self.local_window_size*3):i+self.local_window_size*3+1] = False
                else:
                # attn_mask2 = attn_mask.unsqueeze(1).to(attn_score.dtype)
                # attn_mask2 = attn_mask2.repeat(n_head, attn_mask2.size(2), 1)
                    if apply_mask == True:
                ...
                if use_memory_mask:
                    ...
                    attn_mask[torch.eye(inp.shape[1]).byte( )] = False
                    for i in range(len(attn_mask)):
                        attn_mask[i, max(0, i-self.local_window_size*3):i] = False
                else:
                    if apply_mask == True:
                        # attn_mask2 = attn_mask.unsqueeze(1).to(attn_score.dtype)
                        # attn_mask2 = attn_mask2.repeat(n_head, attn_mask2.size(2), 1)
                ...
                if use_memory_mask:
                    ...
                    for i in range(0, inp.shape[1]//(self.local_window_size*3)+1):
                        attn_mask[i*self.local_window_size*3:(i+1)*self.local_window_size*3, i*self.local_window_size*3:(i+1)*self.local_window_size*3] = False
                else:
                    if apply_mask == True:
                        if inp.shape[1] in self.attn_mask:
                            attn_mask = self.attn_mask[inp.shape[1]].to(attn_score.device)
                ...
                if use_memory_mask:
                    if inp.shape[1] == self.local_window_size*3:
                        attn_mask = torch.zeros((inp.shape[1], inp.shape[1])).byte().to(attn_score.device)
                    else:
                        start_loc = random.choice([i for i in range(self.local_window_size*3)])
                        attn_mask = torch.ones((inp.shape[1], inp.shape[1])).byte().to(attn_score.device)
                        attn_mask[0:start_loc, 0:start_loc] = False
                        for i in range(0, inp.shape[1]//self.local_window_size):
                            attn_mask[(i*self.local_window_size*3)+start_loc:((i+1)*self.local_window_size*3)+start_loc, (i*self.local_window_size*3)+start_loc:((i+1)*self.local_window_size*3)+start_loc] = False
                else:
                    if apply_mask == True:
                        if inp.shape[1] in self.attn_mask:
