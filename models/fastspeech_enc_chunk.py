...
class FastSpeech(nn.Module):
    def __init__(self, n_mel_channels,
                 ...
                 n_speakers, speaker_emb_weight, mask_type, local_window_size):
    ...
    def forward(self, inputs, use_gt_pitch=True, pace=1.0, max_duration=75, apply_mask=True):
        ...
        enc_out, enc_mask = self.encoder(mfcc, mel_lens, apply_mask=apply_mask)
        dec_out, dec_mask = self.decoder(enc_out, mel_lens, apply_mask=apply_mask)
...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, mask_type, local_window_size, dropatt=0.1,
                 pre_lnorm=False):
    ...
    def forward(self, inp, attn_mask=None, apply_mask=True):
        return self._forward(inp, attn_mask, apply_mask=apply_mask)

    def _forward(self, inp, attn_mask=None, apply_mask=True):
        residual = inp
            ...
            if self.mask_type == 'default':
                ...
                if apply_mask == True:
                    if inp.shape[1] in self.attn_mask:
                        ...
                        attn_mask = torch.ones((inp.shape[1], inp.shape[1])).byte()
...
class TransformerLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, kernel_size, dropout, mask_type, local_window_size,
                 **kwargs):
        super(TransformerLayer, self).__init__()

        self.dec_attn = MultiHeadAttn(n_head, d_model, d_head, dropout, mask_type, local_window_size, **kwargs)
        self.pos_ff = PositionwiseConvFF(d_model, d_inner, kernel_size, dropout,
                                         pre_lnorm=kwargs.get('pre_lnorm'))

    def forward(self, dec_inp, mask=None, apply_mask=True):
        output = self.dec_attn(dec_inp, attn_mask=~mask.squeeze(2), apply_mask=apply_mask)
        output *= mask
...
class FFTransformer(nn.Module):
    def __init__(self, n_layer, n_head, d_model, d_head, d_inner, kernel_size, mask_type, local_window_size,
        ...
        for _ in range(n_layer):
            self.layers.append(
                TransformerLayer(
                    n_head, d_model, d_head, d_inner, kernel_size, dropout, mask_type, local_window_size,
                    dropatt=dropatt, pre_lnorm=pre_lnorm)
            )

    def forward(self, dec_inp, seq_lens=None, conditioning=0, apply_mask=True):
        ...
        for layer in self.layers:
            out = layer(out, mask=mask, apply_mask=apply_mask)
