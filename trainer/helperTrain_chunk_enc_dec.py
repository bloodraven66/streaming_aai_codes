...
class Operate():
    ...
    def autoregressive_decoding(self, loader):
        self.model.eval()
        with torch.no_grad():
            eloss_, mloss_ = 0, 0
            cc, rmse = [], []
            ...
            with tqdm(loader, unit="batch") as tepoch:
                for counter, data in enumerate(tepoch):
                    mfcc, ema, phon, dur, emaLengths, sp_id = self.set_device(data, ignoreList=[0, 1, 2, 3])
                    ...
                    for iter_idx in range(0, mfcc.shape[1]//self.chunk_size):
                        chunk = mfcc[:, :(iter_idx+1)*self.chunk_size]
                        if iter_idx == 0:
                            ema_chunk = torch.zeros((mfcc.shape[0], self.chunk_size, 12))
                        emaOut = self.model((chunk.to(emaLengths.device).float(), ema_chunk.to(emaLengths.device).float(), emaLengths, sp_id.long()), src_pad=False).cpu()
                        ema_chunk = torch.cat([ema_chunk, emaOut[:, -self.chunk_size:, :]], dim=1)
                        del chunk
                    del ema_chunk
                    # ema_target_shifted = torch.cat([torch.zeros((ema.shape[0], self.chunk_size, ema.shape[-1])).to(ema.device), ema[:, self.chunk_size:, :]], dim=1)
                    minLens = []
                    lengths_for_ema = []
                    ema = ema.to(emaLengths.device)
                    emaOut = emaOut.to(emaLengths.device)
    ...
    def trainloop(self, loader, mode):
        ...
        with tqdm(loader, unit="batch") as tepoch:
            for counter, data in enumerate(tepoch):
                ...
                ema_target_shifted = torch.cat([torch.zeros((ema.shape[0], self.chunk_size, ema.shape[-1])).to(ema.device), ema[:, self.chunk_size:, :]], dim=1)
                # print(ema.shape, ema_target_shifted.shape)
                # exit()
    ...
    def trainer(self, model, loaders):
        ...
        for epoch in range(int(self.config.common.numEpochs)):
            ...
            self.autoregressive_decoding(valLoader)
